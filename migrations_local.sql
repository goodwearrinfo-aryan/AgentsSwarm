-- Local development migrations (plain PostgreSQL, no Supabase auth)

-- Create authenticated role for RLS policies
CREATE ROLE authenticated NOINHERIT;
GRANT authenticated TO nexsus;
...
CREATE POLICY users_isolation ON users
    FOR ALL
    TO authenticated
    USING ((current_user) = id::text)
    WITH CHECK ((current_user) = id::text);
...
CREATE POLICY swarm_runs_isolation ON swarm_runs
    ...
    WITH CHECK ((current_user) = user_id::text);
...
CREATE POLICY memory_entities_isolation ON memory_entities
    ...
    USING (
        EXISTS (
            ...
            AND (current_user) = me.user_id::text
        )
    );
...
CREATE POLICY graph_nodes_isolation ON memory_graph_nodes
    ...
    USING (
        EXISTS (
            ...
            AND (current_user) = sr.user_id::text
        )
    );
...
-- Skip pg_cron for local dev - memory decay can be run manually or via scheduler
